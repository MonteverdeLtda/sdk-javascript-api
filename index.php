...
(function (jQmv) {
	jQmv.fn.serializeFormJSON = function () {
		var o = {};
		var a = this.serializeArray();
		jQmv.each(a, function () {
			if (o[this.name]) {
				if (!o[this.name].push) {
...
(function (window) {
	...
	Mv.init = ((options) => {
		...
		try {
			if(!jQmv.fn.jqmv){
				throw new Error('jQuery no existe.');
			}else{
				console.log('jQuery Version: ' + $.fn.jqmv);
			}
	...
	Mv.postLogInManual = ((params, elmAlerts, callback) => {
		_callback = callback || callback_active;
		try {
			if(!params['username-login-monteverde']){
				throw new Error('username no recibido en login.');
			}
			if(!params['password-login-monteverde']){
				throw new Error('password no recibido en login.');
			}
			if(!elmAlerts){
				throw new Error('elmAlerts no recibido en login.');
			}
		} catch (e) {
			console.error(e);
			return false;
			...
			send = {
				"username": params['username-login-monteverde'],
				"password": params['password-login-monteverde']
			};
			
			Mv.api.post('/login', send)
			.then(a => {
				var tkn = undefined;
				if(a.config.headers[Mv._config.cookie.headerName] != undefined){ tkn = a.config.headers[Mv._config.cookie.headerName]; } 
				else { if(Mv.cookie.read(Mv._config.cookie.cookieName) != ''){ tkn = Mv.cookie.read(Mv._config.cookie.cookieName); }; };
				
				if(a.status == 200 && a.data.id != undefined && a.data.id > 0){
					localStorage.setItem('_userid', a.data.id);
					Mv.Modal.eventFire('FG-login-modal-messages', 'click');
					jQmv(elmAlerts).html('Registrando usuario en la sesion.');
				}else{
					jQmv(elmAlerts).html('Usuario no detectado en DB.');
				}
			})
			.catch(function (e) {
				// console.log("Respuesta Login. K.O.");
				if(e.respons != undefined && e.response.data != undefined && e.response.data.code != undefined && e.response.data.code == 1012){
					jQmv(elmAlerts).html('Datos Invalidos.');
					
				}else{
					jQmv(elmAlerts).html('Error de respuesta de ingreso.');
			...
			.finally(function () {
				// console.log("Respuesta Login. finally.");
				jQmv(elmAlerts).html('Procesando datos...');
				
				Mv.checkSession();
				Mv.refreshUser(_callback);
			});
		}
	});
	
	Mv.userData = (() => {
		return Mv._userData.authResponse.userData;
	});
	
	Mv.createFormLogin = ((params, call) => {
		try {
			console.log(params);
			if(!params.elementId){ throw new Error('Falta el elementId en los parametros.'); };
			if(!params.elementAlerts){ throw new Error('Falta el elementAlerts en los parametros.'); };
		}
		catch(e){
			console.error(e);
			return false;
			...
			var optionsForm = {};
			optionsForm.debug = true;
			if(params.errorLabelContainer != undefined){ optionsForm.errorLabelContainer = params.errorLabelContainer; };
			optionsForm.wrapper = 'span';
			optionsForm.ignore = [];
			optionsForm.errorElement = 'span';			
			optionsForm.rules = {
				"username-login-monteverde": {
					required: true,
					minlength: 4
				},
				"password-login-monteverde": {
					required: true,
					minlength: 4
				},
			};
			
			optionsForm.messages = {
				"username-login-monteverde": {
					required: '<i class="fa fa-times-circle"></i> Ingrese su usuario Monteverde.',
					minlength: '<i class="fa fa-times-circle"></i> El usuario no es valido.'
				},
				"password-login-monteverde": {
					required: '<i class="fa fa-times-circle"></i> Ingrese su contraseña personal.',
					minlength: '<i class="fa fa-times-circle"></i> La contraseña es muy corta.'
				},
			};
			
			optionsForm.submitHandler = function(frm) {
				var data = jQmv(frm).serializeFormJSON();
				Mv.postLogInManual(data, params.elementAlerts, call);
				/* 
				var userName = $('#username-login-monteverde').val();
				var userHash = $("#password-login-monteverde").val();
				
				var elmStatus = document.getElementById('status');
				call
				try {
					elmStatus.innerHTML = ('Validando datos de login manual.');
				}
				catch (e) {
					elmStatus.innerHTML = ('Error.');
				}
				finally {
					elmStatus.innerHTML = ('Finalizado.');
				};*/
			};
			optionsForm.onsubmit = true;
			
			jQmv(params.elementId).validate(optionsForm);
		}
	})
