...
try {
	...
	try {
		...
		console.log('Variable $ no se pudo definir');
	}
}
*/
(function (window) {
	...
	Mv.orm = {
	  methods: {
		readRecord: function () {
		  ...
		  this.record = null;
		  var self = this;
		  Mv.api.get('/records/' + this.subject + '/' + this.id).then(function (response) {
		...
		readRecords: function () {
		  ...
		  var self = this;
		  Mv.api.get(url).then(function (response) {
		...
		readOptions: function() {
		  ...
		  for (var key in this.references) {
			...
			if (subject !== false) {
			  ...
			  Mv.api.get('/records/' + subject + '?include=' + primaryKey + ',' + displayColumn).then(function (subject, primaryKey, displayColumn, response) {
		...
		updateRecord: function () {
		  Mv.api.put('/records/' + this.subject + '/' + this.id, this.record).then(function (response) {
		...
		createRecord: function() {
		  var self = this;
		  Mv.api.post('/records/' + this.subject, this.record).then(function (response) {
		...
		deleteRecord: function () {
		  Mv.api.delete('/records/' + this.subject + '/' + this.id).then(function (response) {
			console.log(response.data);
		  }).catch(function (error) {
			console.log(error);
		  });
	...
	Mv.ormRegister = {
	  methods: {
		readRecord: function () {
		  ...
		  this.record = null;
		  var self = this;
		  Mv.api.get('/register.php/records/' + this.subject + '/' + this.id).then(function (response) {
			self.record = response.data;
		  }).catch(function (error) {
			console.log(error);
		  });
		...
		readRecords: function () {
		  this.subject = this.$route.params.subject;
		  this.records = null;
		  var url = '/register.php/records/' + this.subject;
		  var params = [];
		  for (var i=0;i<this.join.length;i++) {
			params.push('join='+this.join[i]);
		  }        
		  ...
		  var self = this;
		  Mv.api.get('/register.php' + url).then(function (response) {
			self.records = response.data.records;
		  }).catch(function (error) {
			console.log(error);
		  });
		...
		readOptions: function() {
		  ...
		  for (var key in this.references) {
			...
			if (subject !== false) {
			  ...
			  var displayColumn = this.getDisplayColumn(Object.keys(properties));
			  var primaryKey = this.getPrimaryKey(properties);
			  Mv.api.get('/register.php/records/' + subject + '?include=' + primaryKey + ',' + displayColumn).then(function (subject, primaryKey, displayColumn, response) {
				self.options[subject] = response.data.records.map(function (record) {
				  return {key: record[primaryKey], value: record[displayColumn]};
				});
				self.$forceUpdate();
		...
		updateRecord: function () {
		  Mv.api.put('/register.php/records/' + this.subject + '/' + this.id, this.record).then(function (response) {
			console.log(response.data);
		  }).catch(function (error) {
			console.log(error);
		  });
		...
		createRecord: function() {
		  var self = this;
		  Mv.api.post('/register.php/records/' + this.subject, this.record).then(function (response) {
			self.record.id = response.data;
		  }).catch(function (error) {
			console.log(error);
		  });
		...
		deleteRecord: function () {
		  Mv.api.delete('/register.php/records/' + this.subject + '/' + this.id).then(function (response) {
			console.log(response.data);
		  }).catch(function (error) {
			console.log(error);
		  });
	...
	Mv.definitionRegister = {};
	
	Mv.translate = {
		language: {
			es: {
				users_guests_pending: {
					...
					labels: {
						...
						updated: {
							text: 'Fecha Última Modificacion',
						},
						names: {
							text: 'Nombres',
						},
						...
						company_position: {
							text: 'Que cargo tienes?',
						},
						notes: {
							text: 'Cuentanos que ayuda necesitas de Monteverde!',
						},
					}
				}
	...
	Mv.register.components = {};
	
	Mv.register.components.menu = VueMv.component('menu-component', {
		mixins: [Mv.util, Mv.ormRegister],
		template: '#menu',
		props: ['subjects'],
		template: `<div>
			<div v-if="subjects!==null" class="nav flex-column nav-pills">
				...
				Crear
				<router-link v-for="subject in subjects" v-bind:to="{name: 'Add', params: {subject: subject.name}}" class="nav-link" :key="subject.name">
					{{ subject.name }}
				</router-link>
			</div>
		</div>`
	...
	Mv.register.pages['Home'] = VueMv.extend({
		mixins: [Mv.util],
		template: `<div>
			<div>Nothing</div>
		</div>`
	...
	Mv.Forms.Modal.create = ((type, action, subjectPatch, optionsFormItems, call) => {
		// if(optionsFormItems.fields == undefined){ new throw Error('fields no esta definido.'); };
		// if(optionsFormItems.messages == undefined){ new throw Error('messages no esta definido.'); };
		
		var menuBase = '<menu-component v-if="definition!==null" :subjects="definition.tags"></menu-component>';
		var orm = null;
		switch(type){
			case 'register':
				console.log('API register.php');
				orm = Mv.ormRegister;
				break;
			case 'api':
				orm = Mv.orm;
		...
		modalId = 'mv-root-register-users-guests-modal';
		html_out = `
		<div>
			<div id="body-` + modalId + `">
				<div class="row">
					<div class="col-md-12">
						<form vnot-on-:submit="createRecord" action="javascript:return false;" id="form-` + modalId + `">
						<router-view :key="fullPath" v-if="definition!==null" :definition="definition">routeView</router-view>
						</form>
					</div>
				</div>
			</div>
		</div>`;
		Mv.Modal.create(modalId, html_out);
		Mv.Modal.open(modalId);
		
		console.log('Monteverde-SDK-container-' + modalId);
		console.log('body-' + modalId);
		
		var componentAdd = VueMv.extend({
			mixins: [Mv.util, orm],
			template: `<div>
				<h2>{{ title }}</h2>
				  <template v-for="(value, key) in record">
					<div class="form-group" v-if="formItems[key] != undefined && formItems[key].visible == true">
					  <label v-bind:for="key" v-if="Mv.translate.language['es'][subject] != undefined && Mv.translate.language['es'][subject].labels[key] != undefined && Mv.translate.language['es'][subject].labels[key].text != undefined">{{ Mv.translate.language['es'][subject].labels[key].text }}</label>
					  <label v-bind:for="key" v-else="">{{ key }}</label>
					  <input v-if="references[key] === false && formItems[key].email == true" class="form-control" v-bind:id="key" v-bind:name="key" type="email" v-model="record[key]" :disabled="key === primaryKey" />
					  <input v-else-if="references[key] === false && formItems[key].password == true" class="form-control" v-bind:id="key" v-bind:name="key" type="password" v-model="record[key]" :disabled="key === primaryKey" />
					  <input v-else-if="references[key] === false && formItems[key].text == true" class="form-control" v-bind:id="key" v-bind:name="key" type="text" v-model="record[key]" :disabled="key === primaryKey" />
					  <textarea v-else-if="references[key] === false && formItems[key].textarea == true" class="form-control" v-bind:id="key" v-bind:name="key" type="text" v-model="record[key]" :disabled="key === primaryKey"></textarea>
					  
					  <select v-else class="form-control" v-bind:id="key" v-bind:name="key" v-model="record[key]">
						<option value=""></option>
			...
			data: function () {
				return {
					...
					record: null,
					title: 'Crear', // Traduccion 
					options: {},
					formItems: optionsFormItems.fields,
					formItemsMessages: optionsFormItems.messages,
					action: action,
					messagesSubmit: '',
				};
			},
			created: function () {
				...
				self.initRecord();
				self.readOptions();
				self.title = Mv.translate.language['es'][subjectPatch].titles.Add;
				...
				console.log(self.options);
					...
					optionsForm.rules = {};
					for (const prop in self.formItems) {
						// console.log(`self.formItems.${prop} = ${self.formItems[prop]}`);
						if(!self.formItems[prop]){} else {
							optionsForm.rules[prop] = {};
							if(self.formItems[prop].required != undefined && self.formItems[prop].required == true){
								console.log(prop + ' Requerido (*)');
								optionsForm.rules[prop].required = true;								
							}
						}
					}
					...
					optionsForm.submitHandler = function(frm) {
						...
						switch(self.action){
							case 'create':
								console.log('API create register submit.');
								apiUrl = null;
								if(type == 'register'){ apiUrl = '/register.php'; }
								else{ apiUrl = '/register.php'; }
								Mv.api.post(apiUrl + '/records/' + subjectPatch, data)
									.then(function (response) {
										console.log('SDK - creado en ' + apiUrl + '. O.K.');
										self.messagesSubmit = 'Tu registro fue creado con exito, espera que el equipo Monteverde se ponga en contacto contigo para que puedas acceder.';
									})
									.catch(function (error) {
										console.log('SDK - creado en ' + apiUrl + '. K.O.');
									})
									.finally(function () {
					...
					optionsForm.messages = self.formItemsMessages;
					
					jQmv('#form-' + modalId).validate(optionsForm);
			},
			computed: {
				properties: function () {
					var self = this;
					return self.getProperties(action, self.subject, self.definition);
				},
				primaryKey: function () {
					var self = this;
					return self.getPrimaryKey(self.properties);
				},
				references: function () {
					var self = this;
					return self.getReferences(self.properties);
				}
			},
			methods: {
				submitButton(){
		...
		new VueMv({
			router: new VueMvRouter({
				linkActiveClass: 'active',
				routes:[
					{ path: location.hash.substr(1), component: componentAdd },
				]
			}),
			data: function () {
				return {
			...
			created: function () {
				...
				switch(type){
					case 'register':
						...
						Mv.api.get('/register.php/openapi').then(function (response) {
						  self.definitionRegister = response.data;
						  self.definition = response.data;
						}).catch(function (error) {
						  console.log('Error: Mv.api.get(/register.php/openapi)');
						  console.log(error);
						});
						break;
					case 'api':
						console.log('API api.php');
						Mv.api.get('/openapi').then(function (response) {
						  self.definitionRegister = response.data;
						  self.definition = response.data;
						}).catch(function (error) {
						  console.log('Error: Mv.api.get(/register.php/openapi)');
			...
			mounted: function(){
				console.log('mounted Mv.AddInRegister');
				console.log(Mv.translate.language['es'][subjectPatch].text);
					
				// console.log(this.$route.params);
			}
		}).$mount('#body-' + modalId);
		
		// /register/:subject/Add
		
	});
	
	Mv.Forms.Modal.Add = ((modal_id, subject, hashUrl) => {
		Mv.Modal.create_link(modal_id);
		Mv.Modal.open(modal_id);
		location.hash = hashUrl;
	});
	
	
	Mv.register.pages['Forms'] = VueMv.extend({
		mixins: [Mv.util, Mv.ormRegister],
		template: `<div>
			<h2>{{ subject }} - add</h2>
	...
	Mv.createFormNewUserGuest = ((options, call) => {
		try {
			Mv.logoutRegister();
			Mv.Forms.Modal.create('register', 'create', 'users_guests_pending', options, call);
		}
		catch(e){
			console.error(e);
			return false;
	...
	Mv.register.pages['Add'] = VueMv.extend({
		mixins: [Mv.util, Mv.ormRegister],
		template: `<div>
			<h2>{{ subject }} - add</h2>
			<form v-on:submit="createRecord">
	...
	Mv.init = ((options) => {
		...
		try {
			...
			modal_id = 'mv-root-register-modal';
			/*
			var menuBase = '<menu-component v-if="definition!==null" :subjects="definition.tags"></menu-component>';
			var routeView = '<router-view :key="$route.fullPath" v-if="definition!==null" :definition="definition"></router-view>';
			
			var rootRegister = document.createElement("div");
			rootRegister.innerHTML = `
			<div>
				<mv:body>
					-- mv:body --
					<div id="mv-root-register-app-register">
						<div class="row">
							<div class="col-md-4">
								MENU
								` + menuBase + `
					...
					<div id="` + modal_id + `" class="Monteverde-SDK-modal">
						<div class="Monteverde-SDK-overlay jsOverlay"></div>
						<div class="Monteverde-SDK-container">
							<div>
								Modal 
								<br>
								` + routeView + `
							</div>
							<button id="Monteverde-SDK-close-` + modal_id + `" class="Monteverde-SDK-close jsModalClose">✕</button>
						</div>
					</div>
					-- mv:body --
				</mv:body>
			</div>`;
			document.getElementsByTagName('body')[0].appendChild(rootRegister);
			
			// Mv.Modal.create_link(modal_id);
			// Mv.Modal.open(modal_id); //  Para abrir la console de registro
			// Mv.Modal.open('mv-root-register-modal');
			
			el = 'mv-root-register'; // 
			tagAppend = 'mv:register:user_guest';
	
			Mv.appRegister = new VueMv({
				el: '#' + modal_id,
				// router: router,
				router: new VueMvRouter({
					linkActiveClass: 'active',
					routes:[
						{ path: '/Add/:subject', component: Mv.register.pages['Forms'], name: 'Forms'},
						// { path: '/', component: Mv.register.pages['Home']},
						// { path: '/mv-root-register-modal/', component: Mv.register.pages['Home']},
						// { path: '/mv-root-register-modal/register/guest', component: Mv.register.pages['Form'], name: 'RegisterGuest'},
				...
				mounted: function(){
					console.log('mounted appRegister');
					console.log(this.$route.params);
				}
			});
		*/
	...
	Mv.Modal = {
		...
		create: function(modal_id = 'form-mv', html_out = 'Monteverde Modal window text'){
			...
			modal__container.id = 'Monteverde-SDK-container-' + modal_id;
	...
	Mv.postLogInManual = ((params, elmAlerts, callback) => {
		...
		try {
			...
			.catch(function (e) {
				console.log("Respuesta Login. K.O.");
				console.log(e.response);
				if (e.response.data != undefined && e.response.data.code != undefined && e.response.data.code != undefined){
					jQmv(elmAlerts).html('Error: ' + e.response.data.code + ' - ' + e.response.data.message);
				} else {
	...
	Mv.logoutRegister = ((reload) =>{
		var self = this;
		Mv.api.post('/register.php/logout', {})
			.then(function (response) {
				// console.log('SDK - Sesion cerrada en el servidor. O.K.');
			})
			.catch(function (error) {
	...
	Mv.createHashDefault = ((hash) => {
/*
		Mv.api_r_g.get('/records/users_guests_pending', send)
		.then(a => {
			var tkn = undefined;
			if(a.status == 200 && a.data > 0){
				jQmv(elmAlerts).html('Usuario creado con exito.');
			}else{
				jQmv(elmAlerts).html('Error X.');
			}
		})
		.catch(function (e) {
			console.log("Respuesta Login. K.O.");
			console.log(e.response);
			if (e.response.data != undefined && e.response.data.code != undefined && e.response.data.code != undefined){
				if(e.response.data.code == 1009){ e.response.data.message = 'Esta cuenta ya existe.'; }
				jQmv(elmAlerts).html('Error: ' + e.response.data.code + ' - ' + e.response.data.message);
			} else {
				jQmv(elmAlerts).html('Error de respuesta de registro invitado.');
			}
			return false;
		})*/
	});

	Mv.postManualRegisterGuest = ((params, elmAlerts, callback) => {
		_callback = callback || callback_active;
		try {
			if(!params['email-register-guest-monteverde']){
				throw new Error('username no recibido en login.');
			}
			if(!params['password-register-guest-monteverde']){
				throw new Error('password no recibido en login.');
			}
			if(!elmAlerts){
				throw new Error('elmAlerts no recibido en login.');
			...
			send = {
				"email": params['email-register-guest-monteverde'],
				"password": params['password-register-guest-monteverde']
			};
			
			Mv.api_r_g.post('/records/users_guests_pending', send)
			.then(a => {
				var tkn = undefined;
				if(a.status == 200 && a.data > 0){
					jQmv(elmAlerts).html('Usuario creado con exito.');
